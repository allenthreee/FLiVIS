...
4. [FAST-LIO-LOCALIZATION](https://github.com/HViktorTsoi/flivis_LOCALIZATION): The integration of FAST-LIO with **Re-localization** function module.
...
[FAST-LIO2: Fast Direct LiDAR-inertial Odometry](doc/flivis_2.pdf)
...
Set ``` pcd_save_enable ``` in launchfile to ``` 1 ```. All the scans (in global frame) will be accumulated and saved to the file ``` flivis/PCD/scans.pcd ``` after the FAST-LIO is terminated. ```pcl_viewer scans.pcd``` can visualize the point clouds.
